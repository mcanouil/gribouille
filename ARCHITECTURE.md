...
| `src/guide/` | Legend and axis configuration, legend-symbol drawing, and the guide layer: entry tables (`entry.typ`) say what to annotate, primitives draw one element each, and compositions lay them out. Imports only from `deps.typ` and `utils/`, never from `render/` or `scale/`; a primitive that needs a scale takes a closure. |
