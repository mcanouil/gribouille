...
#let entries-of(prim, inherited) = {
  let own = prim.at("entries", default: auto)
  if own != auto { return own }
  if inherited == auto or inherited == none { () } else { inherited }
}

// Guard a tier name at the boundary, so a typo names the primitive rather than
// resolving a surface that does not exist.
