...
///!   ticks or the labels reserve anything, which is why it is decided from the
///!   band rather than from whether two neighbours both draw. An axis with ticks
///!   and no labels has always reserved it, and an axis with labels and no ticks
///!   has always drawn into it.
...
//
// `band` is the depth the ticks and the labels take together, which the caller
// has already measured and which decides the gap that holds them off the edge.
// `tiers` are the tick weights the axis draws, `stack-gap` separates the label
// rows of a stacked axis, and the three title numbers are the gap before the
// title, the box it lands in, and the pad past it.
#let axis-node(
  entries: (),
  rows: (),
  tiers: ("major",),
  ...
  let title-parts = if title == none { () } else {
    (
  ...
  train(compose-stack(
    ...
    // that draws ticks and no labels still reserves it, and one that draws
    // labels and no ticks still draws into it.
    ..if band > 0 and band-gap > 0 { (prim-spacer(band-gap),) } else { () },
    ..label-parts,
    ..title-parts,
    entries: entries,
