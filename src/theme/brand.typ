...
/// resolve one on your behalf. Read the file yourself and pass the result.
///
/// The brand's `color.foreground`, `color.background` and `color.primary`
/// become the theme's `ink`, `paper` and `accent`; every other surface is
/// derived from that pair the way \@theme-minimal derives it, so gridlines and
/// strips re-tint themselves and stay legible in either mode. The remaining
/// data-ink roles (`secondary`, `tertiary`, `success`, `info`, `warning`,
/// `danger`) are de-duplicated in that order into the discrete `palette` every
/// colour and fill scale falls back to.
///
/// Semantic colours may be hex strings, names of `color.palette` entries,
/// aliases of other entries, or `light` / `dark` variants; all of these
/// resolve. A role the brand omits keeps the library default, but a role that
/// is present and malformed fails. `color.light`, `color.dark`, `logo`, `meta`
/// and `defaults` are ignored, as are all font properties other than the
/// family name: Typst can only use a family already available to the compiler.
///
/// Nothing checks that the brand's foreground and background contrast, so a
/// brand that pairs two dark colours yields a theme you cannot read.
///
/// \@category Themes
...
///
/// \@param palette Discrete palette override. `auto` derives one from the brand's data-ink roles; `none` keeps the library default (Okabe-Ito); an array of colours is used as-is.
///
...
///
/// \@examples Brand chrome and a palette derived from the brand's data-ink
/// roles.
/// ```
/// //| alt: "Scatter plot of y against x on a cream canvas with dark ink, points coloured by group from a palette derived from the brand's semantic colours."
/// #let brand = (
///   color: (
...
/// ```
///
/// \@examples Take the `dark` side of every colour variant the brand declares.
/// ```
/// //| alt: "The same scatter plot on the brand's dark variant: light ink and points on a near-black canvas."
/// #let brand = (
///   color: (
...
/// ```
///
/// \@examples Keep the brand's chrome but draw the data in the library's
/// colour-vision-deficiency-safe default.
/// ```
/// //| alt: "The same scatter plot with brand chrome but points coloured from the Okabe-Ito default palette instead of the brand's own."
/// #let brand = (
///   color: (
///     foreground: "#1A1A1A",
///     background: "#FFFAF0",
...
/// ```
///
/// \@examples-static Read the brand file yourself and set the theme once for
/// the whole document. The path resolves against the file calling `yaml`, so
/// this only works where you write it, not inside the package.
/// ```typ
/// #theme-set(theme-brand(yaml("_brand.yml")))
/// ```
///
/// \@see \@theme-minimal, \@theme, \@theme-set
