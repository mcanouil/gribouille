...
///
/// Pass the result to the `arrow` parameter of a line geom, where it marks
/// the direction the line runs, or of a text geom, where it marks the
/// connector running to its data point. The head takes the colour and
/// thickness of the line it sits on, and stays solid under a dashed
/// `linetype`.
///
/// \@category Helpers
/// \@stability stable
...
///
/// \@param angle Half-opening of the head, in `(0deg, 90deg)`. Smaller values
///   give a narrower head.
///
/// \@param ends Which end of the line carries a head: `"last"`, `"first"`, or
///   `"both"`. On a multi-point geom the ends are the ends of the whole group,
///   not of every join.
///
/// \@param type `"open"` strokes a V; `"closed"` fills a triangle.
///
/// \@returns Arrow specification dictionary.
///
/// \@examples Three segments carrying an open head at their far end.
/// ```
/// //| alt: "Three straight line segments, each ending in an open V-shaped arrowhead pointing away from its start point."
/// #let d = (
...
/// ```
///
/// \@examples A closed head at both ends, marking a span rather than a
/// direction.
/// ```
/// //| alt: "Single horizontal line spanning two values with a filled triangular arrowhead at each end."
...
/// ```
///
/// \@see \@geom-segment, \@geom-curve, \@geom-spoke, \@geom-path, \@geom-line, \@geom-step, \@geom-text, \@geom-label, \@geom-typst
