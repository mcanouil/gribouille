...
///
/// \@param expand Padding around the domain. Accepts a `ratio` (`5%`) for proportional breathing room, a `length` (`5pt`) for canvas-space padding, a `relative` (`5pt + 5%`) for both, or a `(lo, hi)` 2-tuple for asymmetric padding, where either element may be `auto` to keep the 60%-of-slot default on that side. `auto` keeps a 60%-of-slot default on each side; `false` collapses to zero.
///
