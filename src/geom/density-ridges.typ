...
///
/// Mapping must provide `x` and `y`. The `scale` parameter sets the tallest
/// ridge's height in y-level units: `1` makes it exactly reach the next
/// bucket's baseline, larger values overlap it.
///
/// \@category Geoms
...
///
/// \@param scale Tallest ridge height in y-level units; values above `1` overlap the ridge behind.
///
...
///
/// \@param trim Whether to restrict each ridge to its bucket's x range instead of letting it decay to the baseline.
///
/// \@param colour Fixed outline colour. `auto` resolves via the colour scale, falling back to the theme `ink`.
///
/// \@param fill Fixed ridge fill. `auto` resolves via the fill scale, falling back to the theme `paper`.
///
...
///
/// \@param stat Statistical transform. `auto` builds \@stat-density-ridges from the parameters above; pass a stat name or stat object to override.
///
...
///
/// \@examples Classic overlapping ridgeline plot of three shifted samples.
/// ```
/// //| alt: "Ridgeline chart with groups a, b, c stacked on the y-axis and values on the x-axis; each group's density ridge overlaps the one behind it."
...
/// ```
///
/// \@examples Map `fill` to the bucket column to colour each ridge.
/// ```
/// //| alt: "Ridgeline chart with groups a, b, c on the y-axis, each ridge filled in its own colour with semi-transparent overlap."
/// #let xs = (1, 2, 2, 3, 3, 3, 4, 4, 5, 7)
/// #let d = ()
/// #for (i, grp) in ("a", "b", "c").enumerate() {
///   for x in xs {
...
/// ```
///
/// \@see \@stat-density-ridges, \@geom-density, \@geom-violin
