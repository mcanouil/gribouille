...
/// directly; construct it to pick a streamgraph `offset`.
///
/// \@category Positions
/// \@stability stable
/// \@since 0.0.1
...
///   `"wiggle"` places each stack at the Byron-Wattenberg baseline that
///   minimises the weighted wiggle of the layer boundaries, the classic
///   streamgraph look. Both shifted offsets suit \@geom-area over a
///   continuous x; neither is supported under `coord-radial`.
///
...
/// ```
///
/// \@examples Silhouette offset: each stack centred on zero (ThemeRiver).
/// ```
/// //| alt: "Streamgraph of three coloured bands flowing symmetrically around the zero line, with band thickness carrying each group's value over x = 0 to 11."
/// #let d = ()
...
/// ```
///
/// \@examples Wiggle offset: the baseline minimising boundary wiggle.
/// ```
/// //| alt: "Streamgraph of three coloured bands on a wiggle-minimising baseline, flowing organically around the zero line over x = 0 to 11."
/// #let d = ()
/// #for (gi, grp) in ("a", "b", "c").enumerate() {
///   for i in range(0, 12) {
///     let wave = calc.sin(i * 0.55 + gi * 1.1) * (0.35 + gi * 0.2)
