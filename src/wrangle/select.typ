...
///
/// Positional string columns select and reorder; other columns are dropped. A
/// selected `as-factor` column keeps its discrete tag.
///
/// \@category Helpers
...
///
/// \@param names Columns to keep, as positional strings.
///
/// \@returns A row-store array carrying only the named columns.
///
/// \@examples-static Keep just the class and highway columns.
/// ```
...
///
/// Each named argument maps a new name to the old column name it replaces
/// (`rename(data, new: "old")`), mirroring the dplyr `rename(new = old)` order.
/// A renamed `as-factor` column keeps its discrete tag under the new name.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
/// \@param data Row-store (array of dicts) or column-store (dict of arrays).
///
/// \@param renames Named `new: "old"` pairs.
///
/// \@returns A row-store array with the columns relabelled in place.
///
/// \@examples-static Rename `hwy` to `highway`.
/// ```
...
///
/// The positional columns move together, in the given order. By default they
/// go to the front; `before:` / `after:` place them just before or after a
/// target column instead. The two anchors are mutually exclusive.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
/// \@param data Row-store (array of dicts) or column-store (dict of arrays).
///
/// \@param cols Columns to move, as positional strings.
///
/// \@param before Target column to insert the moved columns before.
///
/// \@param after Target column to insert the moved columns after.
///
/// \@returns A row-store array with the columns reordered.
///
/// \@examples-static Bring `class` to the front.
/// ```
...
///
/// With no columns, a row is dropped when any of its columns is missing; with
/// positional columns, only those are inspected. A value is missing when it is
/// `none` (an absent key reads as `none`).
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
/// \@param data Row-store (array of dicts) or column-store (dict of arrays).
///
/// \@param cols Columns to inspect, as positional strings; none inspects all.
///
/// \@returns A row-store array without the dropped rows.
///
/// \@examples-static Drop rows missing a highway figure.
/// ```
...
///
/// Positional columns form the key; the first row seen for each key is kept,
/// in first-appearance order, with every column retained. With no columns the
/// whole row is the key (native `.dedup()`).
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
/// \@param data Row-store (array of dicts) or column-store (dict of arrays).
///
/// \@param cols Key columns, as positional strings; none keys on the whole row.
///
/// \@returns A row-store array with duplicate keys removed.
///
/// \@examples-static One representative row per class.
/// ```
...
///
/// Numeric ordering: cell values are parsed as numbers. Rows whose `col` is
/// non-numeric or missing sort last, so they are kept only when `n` exceeds the
/// numeric rows. `by:` ranks within each group (`none` for the whole dataset).
/// Up to `n` rows are kept per group, ties keeping input order (no tie
/// expansion).
///
/// \@category Helpers
/// \@subcategory Data wrangling
...
/// \@param data Row-store (array of dicts) or column-store (dict of arrays).
///
/// \@param col Column to rank by.
///
/// \@param n Number of rows to keep per group.
///
/// \@param by Grouping columns: `none`, a column name, or an array of names.
///
/// \@returns A row-store array of the top rows.
///
/// \@examples-static The thirstiest car in each class.
/// ```
...
///
/// The bottom-n counterpart of \@slice-max, with the same parsing, grouping,
/// and no-tie-expansion rules (non-numeric or missing `col` sorts last).
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
///
/// \@examples-static The most economical car in each class.
/// ```
