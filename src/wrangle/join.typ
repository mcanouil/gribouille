...
///
/// Rows match on the `by:` key columns (a natural join on the common columns
/// when `by: none`); key values compare by their string form, so a native `1`
/// matches the string `"1"`. Output columns are x's columns, then y's non-key
/// columns. An x row with no match keeps `none` in the y columns; multiple y
/// matches emit one output row each (x order then y match order). A non-key
...
///
/// \@param y Right dataset (row-store or column-store).
///
/// \@param by Key columns: `none` for the common columns, a name, or an array.
///
/// \@returns A row-store array of x augmented with y's columns.
///
/// \@examples-static Attach each customer's city to their orders.
/// ```
/// #let joined = left-join(orders, customers, by: "customer")
/// ```
///
/// \@see \@inner-join, \@full-join
...
///
/// Like \@left-join but drops x rows with no match; output columns are x's
/// columns then y's non-key columns.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
///
/// Matched rows combine as in \@left-join; a y row with no match becomes a new
/// output row with its key columns and `none` in x's non-key columns.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
///
/// A filtering join: the output is a subset of `x` (columns unchanged),
/// restricted to rows whose key appears in `y`. Keys compare by their string
/// form, as in \@left-join.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
///
/// \@param y Dataset whose keys gate `x` (row-store or column-store).
///
/// \@param by Key columns: `none` for the common columns, a name, or an array.
///
...
/// \@param x Dataset to filter (row-store or column-store).
///
/// \@param y Dataset whose keys exclude rows of `x` (row-store or column-store).
///
/// \@param by Key columns: `none` for the common columns, a name, or an array.
///
...
///
/// The output columns are the union of every input's columns, in first-
/// appearance order; a row missing a column fills it with `none`. Factor tags
/// are unioned across the inputs. Passing no datasets returns `()`, and a
/// `none` dataset is skipped.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
///
/// \@examples-static Combine two batches of rows with aligned columns.
/// ```
...
///
/// The inputs are matched by row position, so every input must have the same
/// number of rows; output row `i` merges the `i`th row of each. Column names
/// must be unique across the inputs. Factor tags merge. Passing no datasets
/// returns `()`, and a `none` dataset is skipped.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
///
/// \@examples-static Attach a computed column block to a dataset.
/// ```
