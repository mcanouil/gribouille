...
///
/// `cols` (a column name or an array of names) are the columns to melt; every
/// other column is kept as an identifier. Each input row becomes one output row
/// per melted column: the kept columns, then `names-to` holding the melted
/// column's name, then `values-to` holding its cell value.
///
/// \@category Helpers
...
///
/// \@param cols Columns to melt, as a string or an array of strings.
///
/// \@param names-to Name of the output column holding the melted column names.
///
/// \@param values-to Name of the output column holding the melted cell values.
///
/// \@returns A long row-store array.
///
/// \@examples-static Melt city and highway economy into one measurement column.
/// ```
...
/// Spread a name column and a value column into one column per name (long to
/// wide).
///
/// `names-from` holds the new column names, `values-from` their cell values;
/// every other column identifies the output row. Rows sharing the identifier
/// columns collapse into one, with a column per distinct `names-from` value
/// (stringified, since dict keys are strings) in first-appearance order. Absent
/// name/identifier combinations are filled with `none`; a repeated combination
/// (the same `names-from` value twice within one identifier group) is an
/// ambiguous spread and fails.
///
/// \@category Helpers
/// \@subcategory Data wrangling
/// \@stability experimental
...
/// \@param data Row-store (array of dicts) or column-store (dict of arrays).
///
/// \@param names-from Column whose values become the new column names.
///
/// \@param values-from Column whose values fill the new columns.
///
/// \@returns A wide row-store array.
///
/// \@examples-static Spread a key column into one column per key.
/// ```
