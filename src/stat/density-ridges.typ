...
///
/// For every distinct y value, emits `n` evenly spaced rows along x where
/// `height` is the estimated density scaled to a shared maximum of 1 across
/// all buckets, plus the after-stat columns `_density` (estimated density),
/// `_scaled` (density scaled per bucket), and `_n` (the bucket's
/// observation count).
///
/// \@category Stats
...
/// per bucket.
///
/// \@param trim Whether to restrict each curve to its bucket's x range.
/// `false` (default) extends the grid by three bandwidths on each side so
/// every ridge decays to its baseline.
///
/// \@returns Statistic object with `name: "density-ridges"`, consumed by \@geom-density-ridges.
///
/// \@examples Sharper ridges via `adjust` on the backing statistic.
/// ```
/// //| alt: "Three overlapping ridgeline densities for groups a, b, c on the y-axis over values on the x-axis, smoothed with a halved bandwidth so each ridge shows two sharp peaks."
...
/// ```
///
/// \@see \@geom-density-ridges, \@stat-density, \@stat-ydensity
