...
///
/// The loess band uses normal quantiles with the residual variance estimated
/// on `n - trace(L)` degrees of freedom (`L` the smoothing operator), a close
/// approximation to R's t-interval construction.
///
/// \@category Stats
...
/// ```
///
/// \@examples A loess curve follows the oscillation the linear fit averages
/// away; `span` tunes how locally it bends.
/// ```
/// //| alt: "Scatter chart with x and y axes overlaid by a loess smoother and shaded 95 percent confidence band following the wave pattern in the points."
