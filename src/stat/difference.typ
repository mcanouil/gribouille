...
///
/// Consumes `x`, `ymin`, and `ymax`, where `ymin` and `ymax` are the two
/// series to compare (either may be on top). Consecutive points whose
/// difference changes sign are joined by the exact linear intersection of
/// the two series, inserted as the last point of one run and the first
/// point of the next, so adjacent ribbon segments share a vertex at the
/// crossover. Each output row carries a `_sign` column holding
/// `levels.at(0)` where `ymax > ymin` and `levels.at(1)` where
/// `ymax < ymin`, and a `group` column separating the runs; map `fill` to
/// `after-stat("_sign")` to colour the band by which series leads.
///
/// \@category Stats
...
///   extend the run in progress; a group that only ever ties takes the
///   first level (its band has zero area either way).
///
/// \@returns Statistic object with `name: "difference"`, consumed by geom
///   layers (usually \@geom-ribbon).
///
/// \@examples Shade the gap between two series by which one is on top.
/// ```
/// //| alt: "Ribbon chart of the band between a sine and a cosine series over x = 0 to 12; the band is filled in one colour where the first series leads and another where it trails, with clean tips at every crossing."
...
/// ```
///
/// \@examples Name the levels and overlay the series themselves.
/// ```
/// //| alt: "Band between a rising forecast series and a wavering actuals series, filled by 'above forecast' and 'below forecast' legend entries, with both series drawn as lines over the band."
...
/// ```
///
/// \@see \@geom-ribbon, \@stat-align, \@after-stat
