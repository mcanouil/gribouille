...
/// Filled 2D density statistic: iso-bands of a Gaussian kernel density
/// estimate.
///
/// Smooths the `(x, y)` sample into a density surface on an `n × n` grid,
/// then cuts the surface into level bands. Emits the same row shape
/// as \@stat-contour-filled (closed polygons with `fill` bound to
/// `_level`), so it pairs with \@geom-density-2d-filled or \@geom-polygon.
///
/// Either `breaks`, `binwidth`, or `bins` controls band placement;
...
/// ```
///
/// \@see \@geom-density-2d-filled, \@stat-contour-filled, \@stat-density-2d
