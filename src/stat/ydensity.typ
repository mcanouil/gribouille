...
///
/// For every distinct x value, emits `n` evenly spaced rows along y where
/// `violinwidth` is the display width in `[0, 1]`, plus the after-stat
/// columns `_density` (estimated density), `_count` (density scaled by the
/// bucket's observation count), `_scaled` (density scaled to a per-bucket
/// maximum of 1), and `_n` (the bucket's observation count).
///
/// Widths are normalised across the buckets of one aesthetic group; with a
/// `colour`/`fill` grouping each group normalises independently.
///
/// \@category Stats
...
/// per bucket.
///
/// \@param trim Whether to restrict each curve to its bucket's y range.
/// `true` (default) trims the tails; `false` extends the grid by three
/// bandwidths on each side.
///
/// \@param scale Width normalisation across buckets. `"area"` (default)
/// divides every density by the largest density so equal areas read as equal
/// mass, `"count"` additionally weights each bucket by its observation count,
/// and `"width"` stretches every bucket to the full width.
///
/// \@returns Statistic object with `name: "ydensity"`, consumed by \@geom-violin.
///
/// \@examples Wider smoothing via `adjust` on the backing statistic.
/// ```
/// //| alt: "Two violins for groups a and b on the x-axis over values on the y-axis, smoothed with a doubled bandwidth so each silhouette is a single wide lobe."
...
/// ```
///
/// \@see \@geom-violin, \@stat-density, \@stat-boxplot
#let stat-ydensity(
  bw: auto,
  adjust: 1,
  n: 512,
