...
///
/// Smooths the `(x, y)` sample into a density surface on an `n × n` grid,
/// then traces the surface's contour lines. Emits the same row shape
/// as \@stat-contour (`x`, `y`, `group`, `_level`), so it pairs
/// with \@geom-density-2d or \@geom-path.
///
...
/// both axes or an `(x, y)` tuple.
///
/// \@param adjust Bandwidth multiplier: `adjust: 0.5` halves the smoothing.
///
/// \@param n Grid resolution per axis: a number or an `(x, y)` tuple.
///
...
///
/// \@examples Density contours of two point clouds via `geom-path`.
/// ```
/// //| alt: "Concentric density contour lines around two point clouds centred near (2, 2) and (6, 5), traced by stat-density-2d driving geom-path."
...
/// ```
///
/// \@see \@geom-density-2d, \@stat-contour, \@stat-density
#let stat-density-2d(
  bw: auto,
  adjust: 1,
  n: 25,
