...
- docs: the `guides()` reference lists each accepted key (`colour`, `fill`, `…`) as a sub-list under the `..args` parameter, sharing one description template; `theme()` keeps its full key table. (#108)
