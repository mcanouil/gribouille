...
- feat!: the summary-dispatch kernel `summarise(name, values)` leaves the public API (renamed to the internal `_apply-summary`), freeing the `summarise` name for the new wrangle verb; the `fun:` argument of `stat-summary` and `stat-summary-bin` (a name string or callable) is unchanged. (#unreleased)
