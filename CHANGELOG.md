...
- fix!: a plot whose tick labels cannot fit the canvas fails with the room they need instead of drawing past its edge. The cm a label reaches past the panel edge is a floor on the margin, but two opposite reaches that together outrun the canvas cannot both be honoured: the right margin is capped against the left so the panel rect cannot invert, the panel floors at zero, and the ink spilled anyway. A three-level discrete x scale with 25-character level names, `guides(y: none)`, and a right legend rendered 102.58pt wide against the 85.04pt it asked for at `width: 3cm`, and fitted its box at `width: 4cm`. Give such a plot more room, rotate the labels with `guides(x: guide-axis(angle: 45))`, shorten them with a `labels:` function on the scale, or shrink them with `theme(axis-text: element-text(size: ...))`. A canvas the axis bands alone fill still draws an empty panel rather than failing, and a shared `compose()` margin still cannot fail a plot that fits on its own. (a9d7399)
- fix!: a legend placed on a plot side, or one `compose()` hoists out of its panels, fits the `width`/`height` it was asked for, and fails with the room it needs when it cannot. The chrome reserved the guide stack without ever capping it against the canvas, so past a minimum footprint the figure grew instead: a 1.2cm by 4cm scatter with a right legend rendered 1.63cm wide, a 6cm by 1.5cm one rendered 1.87cm tall, and a 3cm by 1.5cm two-panel `compose()` with a right legend rendered 1.54cm tall. A legend that used to spill into bare margin now fails rather than shipping a figure that outgrew its box; give the plot more room, move the legend to a shorter side, or shrink it with `guide-legend(nrow:/ncolumn:)`. An inside-panel legend is unaffected, and a canvas the axes alone fill still draws an empty panel rather than failing. (#255)
...
- fix: a plot fits the `width`/`height` it was asked for when its outermost tick label reaches past the panel edge. A tick label is centred on its break, so the break nearest an edge puts half the label outside the panel, and the chrome reserved the label band perpendicular to its own axis but nothing along it: a six-point scatter at 14mm by 15mm rendered 41.13 x 44.74pt against a requested 39.69 x 42.52pt, and two panels in a 30mm by 15mm `compose()` rendered 88.32pt wide against 85.04pt. The reach is now a floor on the margin, net of the band opposite, so the panel shrinks and the figure keeps its size. A plot with room is laid out exactly as before, because the expansion gap already covers the reach; the plots that move are the ones that were overrunning, most visibly under `expand: false`, where a break sits flush on the panel edge. (#258)
- fix: a faceted plot fits the `width`/`height` it was asked for. A strip band was a fixed text-sized cost the panel grid paid before the panels got anything, capped against nothing, so a `facet-wrap` over three levels under `theme-void()` rendered 1.4cm tall in a 1cm square canvas and level names wider than their panel ran off the side of the grid. The bands are budgeted against the grid now, giving up their fixed base and then the whitespace between panels, and a label too wide for the panel it names wraps into it. A plot with room to spare is laid out exactly as before; one whose strip labels cannot fit at all fails with the room they need, the way an oversized axis title does. (#256)
...
- docs: the Typst-markup examples on `typst()` and `element-typst()` write a mean as `$macron(x)$`. They used `$bar(x)$`, which is the vertical-bar symbol rather than the overbar their alt text described, so the rendered title read `Mean |(x) over Time` on the pinned compiler and `Mean |x| over Time` on a newer one. (#253)
