...
- fix: legend entry labels, vertical colourbar tick labels, and y-axis tick labels centre on the cap-height/baseline band instead of the glyph bounds, so a label carrying a descender (`Chinstrap`) no longer sits higher than one without (`Adelie`). (#208)
