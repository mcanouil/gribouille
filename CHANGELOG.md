...
- fix: a discrete `fill` or `colour` mapped to the same column as `x` (e.g., `aes(x: "class", fill: "class")`) colours each `geom-boxplot` box per group instead of rendering them all in the ink colour. (#31)
