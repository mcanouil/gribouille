...
- fix: a `coord-radial()` plot fits the `width`/`height` it was asked for. Its theta tick labels ring the circle just outside the panel, and no margin reserved room for them, so the labels grew the rendered figure past the requested size, by 0.21cm at the top of a 6cm by 5cm clock. Every margin now holds that band, and the circle shrinks to pay for it instead of the figure growing. (#237)
