...
- fix: a faceted `coord-radial()` plot fits the `width`/`height` it was asked for. Its shared axis titles were pushed out by a tick and label band a radial panel never draws, and the chrome had reserved nothing for, so a faceted pie 10cm wide rendered 11.1cm. The titles now sit against the panel grid, as a single radial plot's already did. (#241)
